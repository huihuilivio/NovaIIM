...
bool PasswordUtils::Verify(std::string_view password, std::string_view stored_hash) {
    ...
    // 解析 iterations
    auto dollar1 = s.find('$');
    if (dollar1 == std::string::npos) return false;
    ...
    if (!FromHex(salt_hex, salt, kSaltLen)) return false;
    if (!FromHex(hash_hex, expected_hash, kHashLen)) return false;
