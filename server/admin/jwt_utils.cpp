...
std::string JwtUtils::Sign(int64_t admin_id, std::string_view secret, int expires_seconds, JwtAlgorithm alg) {
...
std::optional<JwtClaims> JwtUtils::Verify(std::string_view token, std::string_view secret, JwtAlgorithm alg) {
    ...
    if (sub_claim && sub_claim->value) {
        std::from_chars(sub_claim->value, sub_claim->value + sub_claim->value_length, result.admin_id);
    }

    // 提取 iat
    auto* iat_claim = l8w8jwt_get_claim(claims, claims_count, "iat", 3);
    if (iat_claim && iat_claim->value) {
        std::from_chars(iat_claim->value, iat_claim->value + iat_claim->value_length, result.iat);
    }

    // 提取 exp
    auto* exp_claim = l8w8jwt_get_claim(claims, claims_count, "exp", 3);
    if (exp_claim && exp_claim->value) {
        std::from_chars(exp_claim->value, exp_claim->value + exp_claim->value_length, result.exp);
    }
