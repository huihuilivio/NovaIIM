...
bool MysqlDbManager::InitSchema() {
    ...
    ok = ok && db.create_datatable<UserDevice>(ormpp_auto_key{"id"}, ormpp_unique{{"uid", "device_id"}});
    ...
    db.execute("CREATE INDEX IF NOT EXISTS idx_session_expires ON admin_sessions(expires_at)");
