...
NovaIIM 是一套跨平台 IM 系统，使用 C++20 实现，包含以下子系统：
...
| Gateway | WebSocket | 可配置 | 与 TCP 并行运行，相同协议帧格式 |
| AdminServer | HTTP JSON | 9091 | 管理面板 API (用户管理/消息管理/审计/仪表盘) |
| FileServer | HTTP REST | 9092 | 文件上传/下载/删除 (小文件/大文件流式/静态预览) |
...
| 消息总线 | msgbus (header-only, MQTT-style 发布-订阅) |
| 构建 | CMake + Ninja, FetchContent 依赖管理 |

## 数据流

```
客户端 (TCP)              管理面板 (HTTP)         文件服务 (HTTP)
    │                          │                      │
    ▼                          ▼                      ▼
 ...
 MPMCQueue ──→ ThreadPool  AuthMiddleware (JWT)       │
                  │              │                    │
                  ▼              ▼                    ▼
               Router        Handlers         REST endpoints
                  │              │         /api/v1/files/upload
    ┌─────────────┼─────────┐    │         /static/** (预览/下载)
    ▼             ▼         ▼    ▼         DELETE /api/v1/files/
 UserSvc      MsgSvc    SyncSvc  DAO Layer      │
 FriendSvc    ConvSvc   GroupSvc     │      本地文件系统
              FileSvc                │
    │             │         │        │
    └──────┬──────┘         │        │
           ▼                │        ▼
...
server/
  ...
  admin/                ← 管理面板: AdminServer/JWT/响应助手/ApiError常量/Handlers
  file/                 ← 文件服务: FileServer (HTTP 文件上传/下载/删除, 大文件流式上传)
  core/events.h         ← MsgBus 事件定义 (Admin↔IM 解耦)
```
