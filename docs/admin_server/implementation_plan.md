...
| # | 任务 | 文件 | 状态 |
|---|------|------|------|
| 0.1 | 统一 JSON 响应助手 | `server/admin/http_helper.h` | 完成 | JsonOk / JsonError / ApiCode |
| 0.2 | 分页请求解析 | `server/admin/http_helper.h` | 完成 | ParsePagination, int64_t Offset() |
| 0.3 | 密码哈希工具 | `server/admin/password_utils.h/cpp` | 完成 | PBKDF2-SHA256, mbedtls 返回值全检查 |
| 0.4 | AdminConfig 扩展 | `server/core/app_config.h` | 完成 | jwt_secret + jwt_expires (Config→AppConfig 已重命名) |
| 0.5 | server.yaml 更新 | `configs/server.yaml` | 完成 | jwt_secret / jwt_expires |
...
| # | 任务 | 文件 | 状态 |
|---|------|------|------|
| 1.1 | User model 补全 | `server/model/types.h` | 完成 | 含 password_hash / created_at / updated_at, ormpp ADL 别名 |
| 1.2 | UserDevice model | `server/model/types.h` | 完成 | |
| 1.3 | AuditLog model | `server/model/types.h` | 完成 | |
| 1.4 | AdminSession model | `server/model/types.h` | 完成 | |
| 1.5 | RBAC models | `server/model/types.h` | 完成 | Role / Permission / RolePermission / UserRole |
...
| # | 任务 | 文件 | 状态 |
|---|------|------|------|
| 1.6 | DaoFactory 抽象工厂 | `server/dao/dao_factory.h/cpp` | 完成 | CreateDaoFactory 根据配置创建对应后端 |
| 1.7 | SqliteDbManager (ormpp+SQLite3) | `server/dao/sqlite3/sqlite_db_manager.h/cpp` | 完成 | WAL + FK + ormpp_auto_key + create_datatable |
| 1.8 | MysqlDbManager (ormpp+MySQL) | `server/dao/mysql/mysql_db_manager.h/cpp` | 完成 | 连接池 + ConnGuard RAII + ping 健康检查 |
| 1.9 | SqliteDaoFactory | `server/dao/sqlite3/sqlite_dao_factory.h/cpp` | 完成 | Pimpl 模式, 持有所有 DAO 实例 |
| 1.10 | MysqlDaoFactory | `server/dao/mysql/mysql_dao_factory.h/cpp` | 完成 | Pimpl 模式, 持有所有 DAO 实例 |
| 1.11 | UserDaoImplT | `server/dao/impl/user_dao_impl.h/cpp` | 完成 | FindByUid / FindById / ListUsers / Insert / UpdateStatus / UpdatePassword / SoftDelete / ListDevicesByUser |
| 1.12 | MessageDaoImplT | `server/dao/impl/message_dao_impl.h/cpp` | 完成 | Insert / GetAfterSeq / UpdateStatus / ListMessages / FindById |
| 1.13 | AuditLogDaoImplT | `server/dao/impl/audit_log_dao_impl.h/cpp` | 完成 | Insert / List (参数化, 多条件组合) |
| 1.14 | AdminSessionDaoImplT | `server/dao/impl/admin_session_dao_impl.h/cpp` | 完成 | Insert / IsRevoked / RevokeByUser / RevokeByTokenHash |
| 1.15 | RbacDaoImplT | `server/dao/impl/rbac_dao_impl.h/cpp` | 完成 | GetUserPermissions / HasPermission (3表JOIN参数化查询) |
| 1.16 | MySQL 客户端库下载 | `cmake/fetch_mysql_client.py` | 完成 | Python 脚本, 系统检测 + cdn.mysql.com 多镜像回退 |
| 1.17 | main.cpp 集成 | `server/main.cpp` | 完成 | CreateDaoFactory → DaoFactory 生命周期管理 |
...
| 4.5 | JWT 单元测试 | Sign → Verify 往返 / 过期 / 篡改 | 完成 | 13用例 |
| 4.6 | 密码哈希测试 | Hash → Verify / 错误密码 | 完成 | 11用例 |
...
| 操作者追踪 | AuditLog.admin_id 明确记录谁在操作 |
| JWT 上下文 | JwtClaims.admin_id 明确管理员身份 |
